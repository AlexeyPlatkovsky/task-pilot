...
Current stages:
- `00_project_profile.md`: create or update `.claude/docs/project_specification.md`
...
- stages 01-04 must stop if `.claude/docs/project_specification.md` is missing
...
If `.claude/docs/project_specification.md` is missing, stages 01-04 must stop and require `00_project_profile.md` first.
...
The project profile is the reusable specification stored at `.claude/docs/project_specification.md`.
...
The project-standard skill format is `.claude/skills/<skill_name>/SKILL.md` using markdown with YAML frontmatter. At minimum, the frontmatter must include `name` and `description`.
...
- store shared project conventions under `.claude/conventions`
- use one file per convention area, such as `.claude/conventions/code.md` or `.claude/conventions/testing.md`
...
- structure docs for selective loading with `.claude/docs/README.md`, purposeful subfolders, and stable section targets
