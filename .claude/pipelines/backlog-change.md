...
control to the manager. When some subjects are grounded and others are not, continue through the
remaining steps for the permitted items and report the refusals. Stop on any other blocked or failed
...
evidence resolves the subject at step 1 without a second identical search and resumes at step 3
carrying its `needs triage` requirement — re-running the same search against unchanged source would
only return `unverified` again and bounce the request indefinitely.
