...
| Major | The change is materially incomplete, under-tested at a required boundary, duplicates a domain model field-for-field in a response schema without an automated sync guard (including response models that extend or compose domain models without guarding against drift), creates a credible maintenance or compatibility risk, or leaves required validation evidence missing. | Must fix and re-review. |
