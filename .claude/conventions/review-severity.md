...
| Major | The change is materially incomplete, under-tested at a required boundary, duplicates a domain model in a response schema without an automated sync guard (covering field-for-field copies, subsets, supersets, and response models that extend, compose, or embed domain models — any structural overlap where future domain field changes would silently break the response), creates a credible maintenance or compatibility risk, or leaves required validation evidence missing. | Must fix and re-review. |
