...
| F005-R8 | All error responses shall return `{"detail": "<message>"}` with appropriate HTTP status codes: 404 for unknown project or item; 400 for domain validation failures (invalid field values rejected by the service layer); 422 for malformed request bodies (FastAPI RequestValidationError); 500 for unexpected errors | must |
...
- **F005-R5:** `PATCH /api/projects/{project_id}/items/{item_id}` with `{"status": "in_progress"}` updates the item file and returns the updated `ItemDetail`. Sending an unknown status value (e.g. `"flying"`) returns HTTP 400 (domain `ValidationFailed`). Sending a malformed body (e.g. an integer where a string is expected) returns HTTP 422 (FastAPI `RequestValidationError`).
...
- **F005-R8:** `GET /api/projects/unknown-project/items` returns HTTP 404 with `{"detail": "Project not found: unknown-project"}`. Sending `{"status": "flying"}` to PATCH returns HTTP 400 (domain validation error). Sending `{"status": 12345}` returns HTTP 422 (body parse error).
