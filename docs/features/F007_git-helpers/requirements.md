...
| F007-R3 | The system shall provide `taskpilot sync pull` that runs `git pull --rebase` and then validates task files | could |
...
- **F007-R3:** `taskpilot sync pull` rebases, then runs validation successfully.
...
- Validation after pull is automatic.
