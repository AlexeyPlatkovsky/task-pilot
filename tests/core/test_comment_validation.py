...
def _comment(
    ...
    item_id: str,
    filename: str,
    created_at: str = "2026-06-23T10:00:00Z",
    *,
    raw: str | None = None,
):
    ...
    (d / filename).write_text(
        raw if raw is not None else GOOD_COMMENT.format(created_at=created_at),
        encoding="utf-8",
    )
...
def test_matching_collision_suffix_filename_is_valid(tmp_path: Path):
    paths = WorkspacePaths.for_root(tmp_path)
    _comment(
        paths, "TP-1", "2026-06-23T10-00-00Z-2.md"
    )  # base stamp still matches created_at
...
def test_filename_created_at_mismatch_is_warning(tmp_path: Path):
    ...
    _comment(
        paths, "TP-1", "2026-06-23T09-00-00Z.md", created_at="2026-06-23T10:00:00Z"
    )
